...
begin
        ...
        repeat
                ...
                gotoxy(10,y+i);
                i:=i+1;
                write('Ââåäèòå ïåðâóþ òî÷êó: ');
                read(a);
                if (a < -1.64240517074131) then a:=-1.64240517074131;
                gotoxy(10,y+i);
                i:=i+1;
                write('Ââåäèòå âòîðóþ òî÷êó: ');
                read(b);
                if (b < -1.64240517074131) then b:=-1.64240517074131;
                gotoxy(10,y+i);
                i:=i+1;
                if b < a then
                        writeln('Ïåðâîå ÷èñëî äîëæíî áûòü ìåíüøå âòîðîãî')
                else
                        check:=true
        until check=true;
                write('Ââåäèòå êîë-âî øàãîâ (îò 1 äî 32767): ');
                readln(m2);
        while (m2 <> trunc(m2)) or (m2 < 1) do
        begin
                gotoxy(10,y+i);
                i:=i+1;
                writeln('×èñëî äîëæíî áûòü öåëûì è áîëüøå 0!');
                gotoxy(10,y+i);
                i:=i+1;
                write('Ââåäèòå øàã (îò 1 äî 32767): ');
                read(m2);
        end;
        m:=trunc(m2);
        flag:=true;
...
begin
        ...
        gotoxy(10,6);
        writeln('Ïîãðåøíîñòü: ', f(b)-f(a)-LT(a,b,m):5:2);
        gotoxy(10,7);
        write('Íàæìèòå ëþáóþ êëàâèøó äëÿ âûõîäà â ãëàâíîå ìåíþ');
        readln();
end;
